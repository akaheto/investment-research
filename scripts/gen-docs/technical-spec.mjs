...
await writeDoc("TECHNICAL_SPEC.docx", [
  ...
  bullet([{ text: "Connector layer: ", bold: true }, { text: "lib/providers/* — one interface per data domain (quotes, history, fundamentals, crypto, macro, news), with concrete free-tier implementations. Provider choice is env-driven (EQUITY_PROVIDER, default yahoo), so upgrading to a paid provider is configuration, not a rewrite. Implemented (B2): EquityProvider (getQuotes batch, getDailyHistory) + registry + yahoo-finance2 v4 adapter with injectable client for offline tests; live behavior verified via the SMOKE=1 test suite. Contract conventions: changePercent is a ratio (0.0123 = +1.23%), bar dates are YYYY-MM-DD oldest-first, all failures wrap in ProviderError{provider, symbol, cause}. Optional EquityProvider.searchSymbol(query) (2026-08-05) resolves free text — a ticker or a company name — to a real symbol via Yahoo's own search, used by addToWatchlist so garbage input is rejected instead of silently becoming a fake instrument." }]),
  bullet([{ text: "Storage: ", bold: true }, { text: "libSQL via Drizzle ORM. Locally a plain file (file:local.db); on Vercel the same client points at Turso (free tier). Identical SQLite semantics both places. db/client.ts accepts either TURSO_DATABASE_URL or DATABASE_URL (2026-08-05) — Vercel's configured var name and the name the code originally expected had drifted apart, which silently broke every production DB call until both names were accepted; see § 7." }]),
  bullet([{ text: "Refresh pipeline & caching: ", bold: true }, { text: "npm run refresh locally; on Vercel a cron job calls /api/refresh, guarded by CRON_SECRET. Connectors are automatically wrapped with TTL-based caching (quotes 15 min, fundamentals 24h, macro 1h, technicals 4h); responses are stored in provider_cache table. Cache layer is transparent to the app — can be disabled with DISABLE_CACHE=1 env var. Expired cache entries are purged on-demand or during refresh." }]),
  ...
  table(["Layer", "Choice", "Why / tradeoff"], [
    ...
    ["Charts", "Recharts (shipped, C3)", "Declarative, fits the dataviz method; price history on the instrument detail page. Revisit for candlesticks if needed."],
  ...
  bullet([{ text: "Turso migrations require explicit statement-breakpoint markers (2026-08-05): ", bold: true }, { text: "drizzle-kit generate inserts \"--> statement-breakpoint\" between statements automatically, but any hand-written migration file must include them too. Local dev's embedded SQLite driver silently tolerates a file without them (executes the whole file as one multi-statement call); Turso's remote HTTP protocol rejects it outright (SQL_MANY_STATEMENTS). This masked two bad migration files until they were run against Turso for the first time." }]),
  bullet([{ text: "Next.js 16 route params are a Promise, including in client components (2026-08-05): ", bold: true }, { text: "a page.tsx under a [param] segment must type params as Promise<{...}> and unwrap it with React's use() (client components) or await (server components). Reading params.foo synchronously silently returns undefined rather than erroring — this broke /instrument/[symbol] and /portfolio/[accountId] completely (always \"not found\") with no build-time or console warning." }]),
  bullet("DATABASE_URL and CRON_SECRET are Vercel \"sensitive\" environment variables — once set, their values can never be read back by the CLI, the dashboard, or a session running with Vercel access. Any operation that needs the real value (running a migration, hitting a secret-gated endpoint) has to execute inside the deployed app itself (an admin action triggered from the browser), never from a local script, however privileged the invoking session is."),
  bullet("Markets page indices/crypto (SPY, QQQ, IWM, BTC, ETH, USDT) are not populated by the normal refresh pipeline — that only fetches prices for whatever's on the watchlist. Those specific instruments need an explicit seed step before the Markets page shows real data; logged in ENHANCEMENTS."),
  bullet("Claude model IDs used server-side (event assessments, narratives) need periodic review against Anthropic's deprecation schedule — claude-opus-4-1-20250805 was found in use with an end-of-life date of the next day (2026-08-05) and replaced with claude-sonnet-5. Nothing currently automates this check."),
]);
